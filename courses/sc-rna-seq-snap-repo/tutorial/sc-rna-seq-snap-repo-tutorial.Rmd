...
logo_file <- file.path(root_dir, "img", "DNB-BINF-Core-logo.png")
...
We assume users have some practical experience using Shell, `git` and `GitHub`.


# How to access and run the snap pipeline

We recommend that users fork the `sc-rna-seq-snap` repo. Team members should use the [stjude-dnb-binfcore](https://github.com/stjude-dnb-binfcore) account, while others can use their preferred GitHub account. We welcome collaborations, so feel free to reach out if you're interested in being added to the `stjude-dnb-binfcore` account.

## Naming of the repo/project

  - Team members: keep the name of the pipeline and add the name of the project at the end, e.g., `sc-rna-seq-snap-Vsx2_SE`.
  - Others: keep the name of the pipeline as it is, unless you plan to use the repo for multiple projects. Then, you should follow the same approach as recommended for team members.

## Privacy repo/project

Please note that the forked repositories are public and contain the code from the main branch of the `sc-rna-seq-snap` repo (code reviewed and tested). User needs to ensure that the main branch of the forked repo is always up to date with `stjude-dnb-binfcore/sc-rna-seq-snap:main`. 
...
Next, open the terminal, navigate to your desired directory, and use `git clone` to clone the repository. For more details, see the [README.md](https://github.com/stjude-dnb-binfcore/sc-rna-seq-snap/blob/main/README.md).
...
Replace the `project_parameters.Config.yaml` with the appropriate file paths and parameters specific to your project.
...
We have generated a Docker image that contain all tools, packages, and dependencies necessary to run the code and analyses modules. The environment is specifically configured for `Rstudio/R v4.4.0` and `Seurat v4.4.0`. For more details, please refer to the [README.md](https://github.com/stjude-dnb-binfcore/sc-rna-seq-snap/blob/main/run-container/README.md).

## CPU and Memory Resources

While we have provided estimates for the computational resources required, users may need to adjust memory settings based on the size of the cohort and the specific demands of the analysis. For reference, the estimated resources are for 8 samples with approximately 50,000 cells in total. For further details, please consult the [README.md](https://github.com/stjude-dnb-binfcore/sc-rna-seq-snap/blob/main/README.md).
